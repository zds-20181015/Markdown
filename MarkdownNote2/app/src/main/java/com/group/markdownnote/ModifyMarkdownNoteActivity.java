...
public class ModifyMarkdownNoteActivity extends AppCompatActivity implements View.OnClickListener {
    private static final String TAG = ModifyMarkdownNoteActivity.class.getName();
    private MyApp mApp;
    private ArrayList<MarkdownNote> noteArrayList;
    private int mId;
    private MarkdownNote mNote;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_modify_markdown_note);
        ...
        if (bundle != null){
            key_position = (int) bundle.getSerializable("key_position");
            mNote = noteArrayList.get(key_position);
        }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.bt_confirm:{
                //修改笔记内容，修改数组noteArrayList
                String title = et_title.getText().toString();
                String content = et_content.getText().toString();
                Date data =new Date();
                MarkdownNote note = new MarkdownNote(mNote.getId(), data, content, title, null);
                noteArrayList.remove(key_position);
                ...
                //返回主页
                ToastUtils.showToast(getApplicationContext(),String.format("修改标题为[%s]的笔记成功！",et_title.getText()));
