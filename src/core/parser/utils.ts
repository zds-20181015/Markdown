...
const validWidthAndHeight = (value: any) => {
  if (!/^\d{1,}$/.test(value)) return ''
  value = parseInt(value)
  return value >= 0 ? value : ''
}

export const lowerPriority = (src: string, offset: number, rules: any) => {
  let i
...
export const getAttributes = (html: any) => {
  const parser = new DOMParser()
  const doc = parser.parseFromString(html, 'text/html')
  const target = (doc.querySelector('body') as HTMLElement).firstElementChild
...
const canOpenEmphasis = (src: any, marker: any, pending: any) => {
...
const canCloseEmphasis = (src: any, offset: any, marker: any) => {
