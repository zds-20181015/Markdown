...
const htmlBlock = (ContentState: any) => {
  ContentState.prototype.createHtmlBlock = function (code: any) {
  ...
  ContentState.prototype.initHtmlBlock = function (block: any) {
    let htmlContent = ''
    const text = block.children[0].text
    const matches = inlineRules.html_tag.exec(text)
    if (matches) {
      const tag: any = matches[3]
  ...
  ContentState.prototype.updateHtmlBlock = function (block: any) {
    const { type } = block
    if (type !== 'li' && type !== 'p') return false
    const { text } = block.children[0]
    const match = HTML_BLOCK_REG.exec(text)
    const tagName: any =
      match && match[1] && HTML_TAGS.find((t: any) => t === match[1])
