...
const imageCtrl = (ContentState: any) => {
  /**
  ...
  ContentState.prototype.insertImage = function ({
    ...
    const { text } = block
    const imageFormat = formats.filter((f: any) => f.type === 'image')
    ...
    if (
      ...
      const imageAlt =
        startOffset !== endOffset ? text.substring(startOffset, endOffset) : alt
      block.text =
  ...
  ContentState.prototype.updateImage = function (
    ...
    const image: any = document.querySelector(`#${imageId} img`)
    if (image) {
      image.click()
  ...
  ContentState.prototype.deleteImage = function ({ key, token }: any) {
    const block = this.getBlock(key)
    const oldText = block.text
    const { start, end } = token.range
  ...
  ContentState.prototype.selectImage = function (imageInfo: any) {
