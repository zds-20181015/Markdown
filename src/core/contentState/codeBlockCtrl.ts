...
const codeBlockCtrl = (ContentState: any) => {
  /**
   * check edit language
   */
  ...
  ContentState.prototype.selectLanguage = function (paragraph: any, lang: any) {
   ...
   */
  ContentState.prototype.updateCodeLanguage = function (block: any, lang: any) {
    ...
    if (block.functionType === 'languageInput') {
      ...
      if (
        ...
        nextSibling.children.forEach((c: any) => (c.lang = lang))
      }
   ...
   */
  ContentState.prototype.codeBlockUpdate = function (
    block: any,
    code = '',
    lang: any
  ) {
   ...
   */
  ContentState.prototype.copyCodeBlock = function (event: any) {
    const { target } = event
    const preEle = target.closest('pre')
    const preBlock = this.getBlock(preEle.id)
    const codeBlock = preBlock.children.find((c: any) => c.type === 'code')
