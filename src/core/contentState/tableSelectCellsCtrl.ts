...
const tableSelectCellsCtrl = (ContentState: any) => {
  ContentState.prototype.handleCellMouseDown = function (event: any) {
    if (event.buttons === 2) {
  ...
  ContentState.prototype.handleCellMouseMove = function (event: any) {
    const { target } = event
    const cell = target.closest('th') || target.closest('td')
  ...
  ContentState.prototype.handleCellMouseUp = function (event: any) {
    const { eventCenter } = this.muya
  ...
  ContentState.prototype.selectTable = function (table: any) {
